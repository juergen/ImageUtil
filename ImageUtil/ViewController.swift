...
class ViewController: NSViewController {
  ...
  @IBAction func update(sender: NSButton) {
  ...
  private func renameImages() {
    ...
    if (useNewStartDate.state == 1 && imageFileData.count > 0) {
      ...
      let dateFirstImage: NSDate = self.getSelectedDate(dateFromSelector, metaData: firstElement)
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), {
      ...
      for image:ImageFileMetaData in self.imageFileData {
        var counter : Int = 1
        var baseDate: NSDate = self.getSelectedDate(self.dateFromSelector, metaData: image)
  ...
  private func getSelectedDate(dateFromSelector:NSMatrix, metaData:ImageFileMetaData) -> NSDate {
    switch (dateFromSelector.selectedRow, metaData.fileNameDate) {
    case (0, _):
      return metaData.imageDate
    case (1, _):
  ...
  private func readMetaDataOfFilesInDirectory(dir:NSURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), {
      for (index, url : NSURL) in enumerate(contents) {
        ...
        println("\(progressFormatted)% \(fileName) \(fileName.parseDateFromFileName())")
